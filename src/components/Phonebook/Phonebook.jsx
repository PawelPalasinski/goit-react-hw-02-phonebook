...
class Phonebook extends Component {
    ...
    handleSubmit = e => {
        ...
        const id = nanoid();
        if (this.state.contacts.filter((el) => el.name === name).length > 0) { return alert(`${name} is already in contacts`); } else {
        return (
        this.setState({
            ...this.state,
    ...
    renderContacts = (filterValue, contactsArray) => {
        if (!filterValue) return contactsArray.map(contact => {
            return (
                    ...
                    <button onClick={() => { this.removeContatcFromState(contact.id) }}>⛌</button>
                    </li>
            );
        });
